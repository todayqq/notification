...
class AuthController extends Controller
{
    ...
    public function postLogin(Request $request)
    {
        ...
        if (Auth::guard('admin')->attempt($credentials)) {
        	if(!Admin::user()->activation_token) {
            	admin_toastr(trans('admin::lang.login_successful'));
            	return redirect()->intended(config('admin.prefix'));
           } else {
                Auth::guard('admin')->logout();
                admin_toastr('你的账号未激活，请检查邮箱中的注册邮件进行激活。', 'warning');
			    return back();
           }
        }
    ...
    public function postRegister(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'username' => 'required|unique:admin_users|min:5|max:30',
            'email' => 'required|email|unique:admin_users|max:255',
            'password' => 'required|min:6|max:30',
            'repass' => 'bail|required|same:password|min:6|max:30'
        ]);
    ...
    {    	    	
    	$user = AdminUsers::where('activation_token', $token)->firstOrFail();

        $user->activation_token = null;
        $user->save();

        if (!Auth::guard('admin')->login($user)) {
        	admin_toastr('恭喜你，激活成功！');
        	return redirect()->intended(config('admin.prefix'));
        }
    }
    ...
    protected function settingForm()
    {
        return Administrator::form(function (Form $form) {
            ...
            $form->image('avatar', trans('admin::lang.avatar'))->rules('max:1')->uniqueName();;
